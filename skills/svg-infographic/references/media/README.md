...
- **Decision**: Selected canonical skin — Candidate C + approved neutral-hierarchy
  adjustment (design review, 2026-08-12).
...
- Status: **generated composite evidence artifact — regenerated profile consumer** (2026-08-12). Mixed light/dark/sketch profiles on one canvas: not an ordinary canonical-output surface, so it is not linted under a single `--palette-profile` (each embedded pilot is linted under its own profile). Package-local automatic regeneration (`skin.mjs contact-sheet`) stays reserved; the recolor/contact-sheet contract items stay partial until it lands. — H-C header, six
