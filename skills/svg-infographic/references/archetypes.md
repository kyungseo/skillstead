...
- **Pill section headers:** each band opens with a filled pill (rounded rect, saturated section color, `class="on-focus"` label, optional ①②③ numbering) at the band's top-left, overlapping or just inside the band's top edge.
...
- **Badges:** number badges (filled circle, on-focus numeral) only when sequence matters; status/corner labels as small pills. Keep every corner decoration ≥ 20–24px from its neighbors.
