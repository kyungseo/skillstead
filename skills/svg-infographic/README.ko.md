...
skill은 렌더링뿐 아니라 두 축으로 결과를 점검합니다:

- **렌더링** — 텍스트 넘침 없음, 한국어/CJK 글자 정상(tofu 없음), SVG와 PNG 크기 일치(PNG 2×), 아이콘 렌더, SVG 편집 가능 유지.
...
이 Claude Code package를 skills 디렉터리에 복사합니다 — **전역**(`~/.claude/skills/`, 모든 프로젝트에서 사용) 또는 **프로젝트**(repo의 `.claude/skills/`, 팀원이 clone만으로 사용) 중 선택:
