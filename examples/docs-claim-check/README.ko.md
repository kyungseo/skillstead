...
[`docs-claim-check`](../../skills/docs-claim-check)의 계약을 검증하기 위한 가상 자료와 날짜별 검증 기록입니다.
검증용 입력 자료는 임의로 만들었습니다. "AcmeTask"는 가상 제품에 흔히 쓰는 Acme 이름을 따르고,
manifest 이름에는 의도적으로 `-fixture` 접미사를 붙였습니다. 실제 프로젝트, 문서나 저장소를
참조하지 않습니다. 검증 기록에서는 실제 공개 문서를 사용한 범위와 한계를 별도로 설명합니다.
