...
_위 preview는 대표 6개를 추린 montage입니다. 아래에 10개 예제 전체를 정리했습니다._
...
Windows·Linux 설치 경로는 [`SKILL.md`](../../skills/svg-infographic/SKILL.md) §6에 문서화되어 있고 Chrome/Edge/Chromium에서 동작할 것으로 예상됩니다. 현재까지 PNG export는 macOS에서 smoke test(10개 예제 전부)했고, Windows/Linux 렌더 검증은 아직 대기 중입니다.

| 환경 | 브라우저 | en/ko SVG → 2× PNG | 상태 |
| --- | --- | --- | --- |
| macOS 15 | Chrome (headless) | 10개 예제 전부 | ✅ 검증 — 정확한 2× 크기, tofu 없음 |
| Windows 10/11 | Chrome / Edge | 문서화된 경로 | ⏳ 예상, 렌더 검증 대기 |
| Linux / WSL | Chrome / Chromium | 문서화된 경로 | ⏳ 예상, 렌더 검증 대기(한국어는 Noto Sans CJK/KR 설치) |
