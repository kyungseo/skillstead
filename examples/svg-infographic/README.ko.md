...
_위 preview는 두 sketch 예제를 hero로 둔 대표 6개 montage입니다. 아래에 14개 예제 전체를 정리했습니다._
...
### 14. Agent system map — sketch 프리셋

**Sketch 프리셋** component architecture: Context, Memory, Tool이 중앙
Orchestrator에 연결되고, 아래 Guardrail과 평가 중 평가 결과가 보정 loop로
되돌아갑니다. 정돈된 손그림 표면이 process flow뿐 아니라 계산된 topology에서도
작동한다는 것을 보여줍니다.
...
번들된 [`scripts/render.sh`](../../skills/svg-infographic/scripts/render.sh)가 Chromium 계열 브라우저를 찾아(macOS·Linux·Windows Git Bash 경로) 렌더하고 PNG 치수까지 검증합니다. 네이티브 PowerShell을 포함한 OS별 수동 명령은 [`references/authoring.md`](../../skills/svg-infographic/references/authoring.md) §8에 있습니다. 현재까지 PNG export는 macOS에서 smoke test(14개 예제 전부)했고, Windows/Linux 렌더 검증은 아직 대기 중입니다.
