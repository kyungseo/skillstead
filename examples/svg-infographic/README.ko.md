...
- [x] 전체 SVG 28개에서 source lint hard error 0건, transform 관련 warning은 2× PNG에서 직접 확인
...
포함된 [`scripts/render.sh`](../../skills/svg-infographic/scripts/render.sh)는 Node.js 18 이상으로
source lint gate를 실행하고, macOS, Linux와 Windows Git Bash에서 Chromium 계열 browser를 찾아 PNG를
만든 뒤 크기를 검증합니다. PowerShell을 포함한 운영체제별 수동 명령은
[`references/authoring.md`](../../skills/svg-infographic/references/authoring.md) §8에 있습니다.
현재 14개 예제의 PNG 출력은 macOS에서만 간이 검증을 마쳤고, Windows와 Linux 검증은 아직
진행하지 않았습니다. 스킬 설치와 발견에는 Node가 필요하지 않습니다. Node가 없으면 Agent가
설치 전에 먼저 승인을 요청합니다. 설치하지 않는 경우에도 전체 수동 원본 점검과 Chromium 직접
실행을 통한 2× PNG 및 화면 검증은 그대로 수행합니다.
