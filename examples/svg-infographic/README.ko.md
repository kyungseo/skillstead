...
_위 preview는 대표 6개를 추린 montage입니다. 아래에 12개 예제 전체를 정리했습니다._
...
번들된 [`scripts/render.sh`](../../skills/svg-infographic/scripts/render.sh)가 Chromium 계열 브라우저를 찾아(macOS·Linux·Windows Git Bash 경로) 렌더하고 PNG 치수까지 검증합니다. 네이티브 PowerShell을 포함한 OS별 수동 명령은 [`references/authoring.md`](../../skills/svg-infographic/references/authoring.md) §8에 있습니다. 현재까지 PNG export는 macOS에서 smoke test(12개 예제 전부)했고, Windows/Linux 렌더 검증은 아직 대기 중입니다.

| 환경 | 브라우저 | en/ko SVG → 2× PNG | 상태 |
| --- | --- | --- | --- |
| macOS 15 | Chrome (headless) | 12개 예제 전부 | ✅ 검증 — 정확한 2× 크기, tofu 없음 |
...
flat하고 구조적인 기술 다이어그램만 다룹니다. 손그림/크레용 "스케치노트" 스타일, 마스코트, 캐릭터 일러스트는 **범위 밖**입니다 — 이 선을 지키는 것이 출력 일관성을 유지하는 방법입니다.
