...
예제 파일을 찾거나 어떤 검사를 거쳤는지 확인할 때 쓰는 목록입니다. 완성된 그림부터 둘러보려면
**[갤러리](../../gallery/index.html)** 를 여세요. 같은 예제 파일과 검증 정보를 보여줍니다.

예제는 생성 방식과 보존 목적에 따라 네 가지로 나뉩니다.
...
9개 타입을 각각 영어·한국어로 만들고, 사용한 패키지와 측정 결과를 담은 생성 기록(receipt)을 함께 둡니다.
패키지 수정으로 그림이나 측정값이 바뀌었는지 이 기록과 대조해 확인합니다.
...
이 중 6개는 갤러리의 대표 예제(★)입니다. 이후 갤러리 교체 작업인 **Wave 2에서 재검토할 대상**이며,
그때 계속 사용할 예제는 당시의 계약에 맞게 갱신하고 그렇지 않은 예제는 내릴 예정입니다.
...
- [시나리오](./preflight/scenarios.md) — 정해진 선택, 구성만 요청한 경우, 별도의 도구 설치 권한을 점검합니다.
- [기대 결과](./preflight/expected-outcomes.md) — 새 실행자에게 주는 입력과 분리합니다. 렌더링이 아닌 다음 행동의 판단을 점검하는 사례입니다.
