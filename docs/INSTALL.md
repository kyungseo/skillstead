...
`street-portrait-artist` also follows the complete-folder rule. Its package contains `CHANGELOG.md`, `LICENSE.txt`,
`SKILL.md`, English/Korean READMEs, `agents/openai.yaml`, and four reference files. Its synthetic gallery and
repository-only behavior fixtures remain under `examples/street-portrait-artist/`.
