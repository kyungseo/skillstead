...
[English](./github-public-release-checklist.md) · **한국어**
...
- [ ] 브랜치가 release까지 이동하는 경로를 확인한다. 예: `feature/* -> develop -> main`
...
- [ ] 공개되는 status 파일에 내부용 next action이 남아 있지 않다.
- [ ] backlog 항목은 public 노출에 안전하며 비밀 정보가 없다.
- [ ] README가 최신이다.
...
- [ ] 공개적으로 알려야 할 한계가 있다면 솔직하게 설명했다.
- [ ] 공개 후 공유할 예정이면 social release note draft가 있다.

공유 문구 초안은 `social-release-note-template.ko.md`를 사용한다.

## 7. Visibility Change

- [ ] 공개 전 점검 결과를 마지막으로 다시 확인한다.
...
- [ ] 즉시 `post-public-verification.ko.md`를 실행한다.
...
- [ ] **release tag ruleset의 applicability를 판정했다**: version tag로 release하는 repo면 적용
      ...
      ruleset이 없으면 심각도를 나눈다. Pinned install, tag-pinned clone·dependency·CI처럼 변경되지 않는
      tag를 전제로 동작하는 중요한 사용 경로가 있으면 `Blocked`, 그 외에는 `Needs attention`으로 두고
      accepted risk와 재검토 조건을 남긴다(`repo-settings-template.ko.md`·이후 릴리스는
      `recurring-release-protection-checkpoint.ko.md` 참조).
...
Ruleset이나 branch protection을 바로 적용하지 않기로 했다면 통과로 숨기지 말고 accepted risk로 기록한다.
예: solo maintainer, 초기 main-only repo, CI 없음. 이 경우 다음 release 전에 다시 검토할 조건을 남긴다.
